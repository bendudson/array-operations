...
The traversal order is unspecified and may be nonlinear."
  (let ((dimensions (ensure-dimensions dimensions)))
    (anaphora:aprog1 (make-array dimensions :element-type element-type)
      (ecase arguments
        ((nil)
         (dotimes (position (array-total-size anaphora:it))
           (setf (row-major-aref anaphora:it position)
                 (funcall function))))
        (:position
         (walk-subscripts (dimensions subscripts position)
           (setf (row-major-aref anaphora:it position) (funcall function position))))
        (:subscripts
...
for invalid and repeated indices.  NOT EXPORTED."
  (anaphora:aprog1 (make-array rank :element-type 'bit :initial-element 0)
    (map nil (lambda (p)
               (assert (and (integerp p) (< -1 p rank)) ()
                       'permutation-invalid-index :index p)
               (assert (zerop (aref anaphora:it p)) ()
                       'permutation-repeated-index :index p)
               (setf (aref anaphora:it p) 1))
         permutation)))
...
(defun invert-permutation (permutation)
  ...
  (coerce (anaphora:aprog1 (make-array (length permutation) :element-type 'fixnum)
            (map nil (let ((index 0))
                       (lambda (p)
                         (setf (aref anaphora:it p) index)
                  (incf index)))
                 permutation))
          'list))
    ...
    (if (identity-permutation? permutation rank)
        ...
        (let ((dimensions (array-dimensions array)))
          (flet ((map-subscripts (subscripts-vector)
            ...
            (anaphora:aprog1 (make-array (map-subscripts (coerce dimensions 'vector))
                                :element-type (array-element-type array))
              (walk-subscripts (dimensions subscripts position)
                (setf (apply #'aref anaphora:it (map-subscripts subscripts))
...
compatibility."
  (anaphora:aprog1 (make-array (array-dimensions array) :element-type element-type)
    (assert (apply #'same-dimensions? array other-arrays))
    (apply #'map-into (flatten anaphora:it) function
      ...
      (let ((inner (ensure-dimensions inner))
        ...
        (if (arrayp object)
            (let ((dimensions (array-dimensions object)))
              (anaphora:aprog1 (make-array (append outer dimensions inner)
                                  :element-type element-type)
                (let* ((outer-size (product outer))
                       (size (product dimensions))
                       (inner-size (product inner))
                       (reshaped (reshape anaphora:it (list outer-size size inner-size))))
